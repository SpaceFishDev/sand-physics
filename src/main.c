...
void process_color(pixel *pixels, int *r, int *g, int *b, int x, int y)
{
	...
	else if (pixels[x + (y * SIZE)].type == STONE)
	{
		*r = 100;
		*g = 100;
		*b = 100;
	...
	if (y > 1 && pixels[x + (y * SIZE)].type != GLOWINGSTUFF)
	{
		*r += 120;
		*g += 120;
		*b += 120;
	...
	if (y < SIZE && pixels[x + (y * SIZE)].type != GLOWINGSTUFF)
	{
		*r += 120;
		*g += 120;
		*b += 120;
	...
	if (x < SIZE && pixels[x + (y * SIZE)].type != GLOWINGSTUFF)
	{
		*r += 120;
		*g += 120;
		*b += 120;
	...
	if (x > 1 && pixels[x + (y * SIZE)].type != GLOWINGSTUFF)
	{
		*r += 120;
		*g += 120;
		*b += 120;
	...
	if (x < SIZE && y < SIZE && pixels[x + (y * SIZE)].type != GLOWINGSTUFF)
	{
		*r += 120;
		*g += 120;
		*b += 120;
	...
	if (x > 1 && y > 1 && pixels[x + (y * SIZE)].type != GLOWINGSTUFF)
	{
		*r += 120;
		*g += 120;
		*b += 120;
	...
	if (x > 1 && y < SIZE && pixels[x + (y * SIZE)].type != GLOWINGSTUFF)
	{
		*r += 120;
		*g += 120;
		*b += 120;
	...
	if (x < SIZE && y > 1 && pixels[x + (y * SIZE)].type != GLOWINGSTUFF)
	{
		*r += 120;
		*g += 120;
		*b += 120;
...
void draw_pixels(SDL_Renderer *renderer, pixel *pixels)
{
	...
	for (int x = 0; x < SIZE; ++x)
	{
		...
		{
			int r, g, b;
			process_color(pixels, &r, &g, &b, x, y);
			r = (r > 255) ? 255 : r;
			g = (g > 255) ? 255 : g;
			b = (b > 255) ? 255 : b;
			...
			if (!complete[x + (y * SIZE)])
			{
				...
				case STONE:
				{
					if (y + 3 < SIZE - 3)
					{
						if (pixels[x + ((y + 3) * SIZE)].type == AIR)
						{
							pixels[x + ((y + 3) * SIZE)].type = STONE;
							pixels[x + ((y)*SIZE)].type = AIR;
							complete[x + ((y + 3) * SIZE)] = 1;
						}
						else if (pixels[x + ((y + 1) * SIZE)].type == WATER)
						{
							pixels[x + ((y + 1) * SIZE)].type = STONE;
							pixels[x + ((y)*SIZE)].type = WATER;
							complete[x + ((y + 1) * SIZE)] = 1;
						}
						else
						{
							if (x + 1 < SIZE - 1 && x - 1 > 0)
							{
								if (pixels[(x + 1) + ((y + 1) * SIZE)].type == AIR && pixels[(x - 1) + ((y + 1) * SIZE)].type == AIR)
								{
									if (randInRange(0, 10000) > 5000)
									{
										pixels[(x + 2) + ((y + 1) * SIZE)].type = STONE;
										complete[(x + 1) + ((y + 1) * SIZE)] = 1;
										pixels[x + (y * SIZE)].type = AIR;
									}
									else
								...
								{
									pixels[(x + 1) + ((y + 1) * SIZE)].type = STONE;
									complete[(x + 1) + ((y + 1) * SIZE)] = 1;
									pixels[x + (y * SIZE)].type = AIR;
								}
								else if (pixels[(x - 1) + ((y + 1) * SIZE)].type == AIR)
...
int main()
{
	...
	SDL_SetWindowTitle(window, "Liam's stupid DIRT game thing");
	pixel *pixels = malloc(SIZE * SIZE * sizeof(pixel));
	for (int i = 0; i != SIZE * SIZE; ++i)
	{
		pixels[i].type = AIR;
	}
	int cursor_type = DIRT;
	int paused = 0;
	while (1)
	{
